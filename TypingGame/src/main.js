   ...
   class App {
    ...
    startTypingGame() {
        ...
        let renderingQuestion = () => {
            ...
            let checked_mustEnterdKeys_index = 0; // チェック済みの問題キー配列のインデックス
            ...
            for(let i = 0; i < questionContent.length; i++) {
                ...
                for(let j = 0; j < romajiArray.length; j++) {
                    ...
                    if(romajiArray[j] == this.typingGame_data.mustEnteredKeys[checked_mustEnterdKeys_index].character) {
                        stateA = this.typingGame_data.mustEnteredKeys[checked_mustEnterdKeys_index].state;
                        if(stateB != 'miss') stateB = stateA;
                        checked_mustEnterdKeys_index++;
                    };
        ...
        if(!this.typingGame_data.mustEnteredKeys.length) {
            generateMustEnterdKeys();
            renderingQuestion();
        }

        // キー入力イベントリスナーを追加（ユーザーの入力から問題の判定結果を行う）
        let eventHandler = {
            typeName: 'keydown',
            callback: event => {
                ...
                // 上のループで見つけた文字と入力された文字列が一致した場合、入力済みの状態を保存
